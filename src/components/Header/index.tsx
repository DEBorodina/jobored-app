...
import { Container, Logo, LogoText, NavbarLink, Navigator } from './styles';

const Header = () => {
  const location = useLocation();

  const isSearchVacancies: boolean =
    location.pathname === ROUTE_NAMES.SEARCH_VACANCIES;
  ...
  return (
    <Container>
      <Logo>
        ...
        <LogoText>{TEXT.JOBORED}</LogoText>
      </Logo>
      <Navigator>
        <NavbarLink
          to={ROUTE_NAMES.SEARCH_VACANCIES}
          $active={isSearchVacancies}
        >
          {TEXT.SEARCH}
        </NavbarLink>
        <NavbarLink to={ROUTE_NAMES.FAVORITES} $active={isFavorites}>
          {TEXT.FAVORITES}
        </NavbarLink>
      </Navigator>
    </Container>
